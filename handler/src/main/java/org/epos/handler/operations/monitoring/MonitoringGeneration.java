...
public class MonitoringGeneration {
	...
	public static JsonArray generate(JsonObject response, JsonObject parameters, HeaderParser header) {
		...
		for(Distribution dx : distributionList) {
			...
			org.epos.handler.beans.Distribution distribution = gson.fromJson(DetailsItemGenerationJPA.generate(detailItem, params, tempHeader), org.epos.handler.beans.Distribution.class);
			...
			for(SoftwareApplication sw : softwareApplicationList) {
				for(String value : sw.getIdentifier().stream().map(Identifier::getIdentifier).collect(Collectors.toList())) {
					if(value.toLowerCase().contains("monitoring/zabbix")) {
						if(dx.getAccessURL()!=null && sw.getRelation().stream().map(LinkedEntity::getUid).collect(Collectors.toList()).containsAll(dx.getAccessURL())){
